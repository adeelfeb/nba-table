...
const FULL_JOB_ACCESS_ROLES = new Set(['hr_admin', 'hr', 'superadmin']);
const READ_JOB_ACCESS_ROLES = new Set(['base_user', 'simple_user', 'admin']);

function hasFullAccess(user) {
  return !!user && FULL_JOB_ACCESS_ROLES.has(user.role);
}

function hasReadAccess(user) {
  if (!user) return false;
  return hasFullAccess(user) || READ_JOB_ACCESS_ROLES.has(user.role);
}
