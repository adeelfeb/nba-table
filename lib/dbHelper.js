...
/**
 * Get database connection status and connection without sending any response.
 * Use this when you want to return empty/polyfill data when DB is disconnected
 * so the frontend keeps working.
 * @returns {Promise<{ connected: boolean, connection?: import('mongoose').Connection }>}
 */
export async function getDBConnection() {
  const dbResult = await connectDB();
  return {
    connected: !!dbResult.success,
    connection: dbResult.success ? dbResult.connection : undefined,
  };
}
